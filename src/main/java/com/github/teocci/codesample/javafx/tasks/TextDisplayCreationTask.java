...
import com.github.teocci.codesample.javafx.elements.SearchHighlightedTextCell;
...
public class TextDisplayCreationTask extends Task<Node>
{
    ...
    protected Node call() throws Exception
    {
        ...
        for (int i = 0; i < N_ITERATIONS; i++) {
            builder.append((i + 1) + ". ").append(LOREM_INPUT).append("\n\n");
            updateProgress(i / 2, N_ITERATIONS);
        }
        LOREM = builder.toString();

        Node textDisplay = createTextDisplay();
        updateProgress(1, 1);
    ...
    private ListView<String> createTextDisplay()
    {
        ...
        textDisplay.setCellFactory(stringListView -> new SearchHighlightedTextCell(searchText));
        return textDisplay;
    }
    ...
    private ObservableList<String> createLinesFromString(String lorem, int wrappingWidth)
    {
        ...
        for (String part : parts) {
            i++;
            double lineCreationProgress = i / (2.0 * parts.length);
            updateProgress(Math.min(1, 0.5 + lineCreationProgress), 1.0);
            if (part.length() == 0) {
                lines.add(part);
                continue;
            }
