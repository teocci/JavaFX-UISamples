 ...
 * the verification mechanism can call back to implementers
 ...
 * In the past we used to pass into the JVM runtime the option "-Djsse.enableSNIExtension=false"
 * which would disable SNI extensions completely, bypassing any handshake checks. With the rewrite
 * of Tradefeeds, this is now problematic, as the setting would apply to all threads executing.
 */
...
public class SSLSkipSNIHostnameVerifier implements HostnameVerifier
{
    ...
    /**
     * We always treat SNI issues as valid. This should only be used in valid and verified
     * cases and not set as the default host name-verifier for all connections
     * (non-Javadoc)
     *
